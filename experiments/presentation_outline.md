...
- ECA 是轻量通道注意力机制。
- 目标是增强模型对有效通道特征的选择能力。
- 本项目将 ECA 加入 YOLO11n 的 P3/P4/P5 特征路径。

实验结果：

| Model | mAP50 | mAP50-95 |
| --- | ---: | ---: |
| YOLO11n baseline | 0.31985 | 0.18066 |
| YOLO11n-ECA | 0.30236 | 0.17121 |

讲述重点：

ECA 单独加入后没有提升性能，说明注意力模块并不一定天然有效，插入位置、训练策略和模型容量都会影响最终结果。这部分可以作为规范消融实验的体现。
...
| YOLO11n-ECA | 通道注意力 | 0.30417 | 0.17239 | 当前设置下未提升 |
| YOLO11n-P2 | 小目标检测头 | 0.33013 | 0.19012 | 当前最佳 |
...
项目不是只展示“有效结果”，也记录了 ECA 未提升的负结果，这让消融实验更完整、更可信。
...
- 当前最佳模型是 YOLO11n-P2。
- P2 相比 baseline 提升了小目标检测效果。
- ECA 单独加入未提升，说明改进模块需要通过实验验证。
