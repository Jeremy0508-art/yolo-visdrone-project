...
本计划用于在现有中文期刊投稿准备之外，新增并推进 IEEE Transactions 英文期刊投稿路线。项目当前采用“双轨投稿准备”：中文稿继续面向《计算机工程与应用》等中文期刊，英文稿面向 IEEE Transactions。两条路线共享真实实验、图表、日志和可复现材料，但稿件结构、语言表达、模板要求、目标读者和投稿材料分别维护。
...
- 不编造未完成实验，不把推测写成结果。
- 不重复投稿；中文稿与 IEEE 英文稿必须在目标期刊、写作结构、贡献表述和投稿时序上保持清晰边界，避免一稿多投或重复发表风险。
...
| 中文稿 | CEA 方向 Word/LaTeX/PDF/审计材料已形成，作为中文期刊投稿路线继续保留和维护 |
...
目标：明确中文期刊路线与 IEEE Transactions 英文路线并行推进，统一实验依据，同时分开维护两套稿件和投稿材料。

动作：

- 新增本计划文档。
- 保留并继续维护 CEA 中文稿、Word/PDF 预览、模板迁移和审计材料。
- 新建 IEEE 独立目录和后续审计文件。
- 更新项目路线图，标记中文期刊与 IEEE 英文期刊为双轨当前路线。
