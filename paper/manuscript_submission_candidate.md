...
无人机航拍图像中目标尺度小、分布密集、遮挡频繁，对实时检测模型的小目标感知和定位能力提出了较高要求。针对 VisDrone 场景下的小目标检测问题，本文以 YOLO11n 为基线，构建并评估了一种结合 P2 高分辨率检测分支、CoordAttention 注意力机制和高分辨率输入的轻量化改进模型。该方法利用 P2 分支增强浅层高分辨率特征表达，引入 CoordAttention 提升位置敏感特征建模能力，并评估 960 输入分辨率与小目标友好数据增强策略的影响。实验结果表明，960 输入分辨率是当前实验设置下的主要增益来源，P2 分支在高分辨率下进一步提升定位质量；YOLO11n-P2-960 在 nano 级轻量模型中达到最佳 mAP50 0.42361 和 mAP50-95 0.25552，高于 YOLO11n-960 的 0.42136 / 0.25067 以及 YOLO11n-P2-CoordAttention-960 的 0.41996 / 0.25174，并保持 55.68 FPS。更大容量的 YOLO11s-960 达到 0.48901 / 0.29812，说明模型容量仍是提升精度的重要因素。该结果说明当前方法更适合解释为高分辨率输入主导、P2 分支有效补充、注意力模块收益有限的精度-复杂度折中，而非对更大模型的全面优势。
...
| YOLO11s baseline 960 | 960 | 9.432 | 21.6 | 0.58964 | 0.47587 | 0.48901 | 0.29812 |
| YOLOv5n baseline | 640 | 2.510 | 7.2 | 0.43588 | 0.32875 | 0.31030 | 0.17513 |
...
表 3 给出了不同模型的复杂度和推理速度。速度结果由 `tools/benchmark_speed.py` 在同一台本地 GPU 上重新测试得到。YOLO11n 的平均延迟为 14.268 ms，FPS 为 70.09；YOLO11n-960 的平均延迟为 16.370 ms，FPS 为 61.09；YOLO11n-P2-960 的平均延迟为 17.960 ms，FPS 为 55.68；YOLO11n-P2-CoordAttention-960 的平均延迟为 19.000 ms，FPS 为 52.63；YOLO11s-960 的平均延迟为 16.096 ms，FPS 为 62.13，但参数量约为 9.432M。高分辨率输入和结构增强都会带来额外推理开销，YOLO11n-P2-960 在 nano 级模型中体现出较好的精度-速度折中。

表 3 不同模型复杂度与推理速度对比

| 方法 | 输入尺寸 | Params/M | GFLOPs | 权重/MB | 平均延迟/ms | FPS |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| YOLOv8n baseline | 640 | 3.013 | 8.2 | 5.95 | 29.578 | 33.81 |
| YOLOv8n baseline 960 | 960 | 3.013 | 8.2 | 5.99 | 14.303 | 69.91 |
| YOLO11s baseline | 640 | 9.432 | 21.6 | 18.28 | 14.227 | 70.29 |
| YOLO11s baseline 960 | 960 | 9.432 | 21.6 | 18.32 | 16.096 | 62.13 |
| YOLOv5n baseline | 640 | 2.510 | 7.2 | 5.02 | 12.996 | 76.95 |
...
本文针对无人机航拍小目标检测问题，在 YOLO11n 基础上构建并评估了结合 P2 高分辨率检测分支、CoordAttention 注意力机制和 960 输入分辨率的改进模型。实验表明，P2 分支能够增强浅层高分辨率特征利用，960 输入分辨率是当前实验中最主要的增益来源。YOLO11n-P2-960 在 nano 级轻量模型中达到最佳 mAP50 0.42361 和 mAP50-95 0.25552，并保持 55.68 FPS 的单图推理速度；YOLO11n-P2-CoordAttention-960 未超过 YOLO11n-P2-960，说明 CoordAttention 在当前高分辨率设置下应解释为辅助设计而非决定性增益来源。YOLO11s-960 达到更高的 0.48901 / 0.29812，表明本文方法的合理定位是轻量模型中的精度-速度-参数量折中，而不是对大容量模型的全面超越。后续工作将进一步探索多分辨率训练、面向小目标的损失函数设计，并在官方评测平台可用后补充测试集结果。
