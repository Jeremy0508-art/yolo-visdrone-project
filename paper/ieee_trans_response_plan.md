...
This document prepares likely IEEE reviewer questions and evidence-bounded response strategies. It should be updated after the final manuscript, TOFC result, UAVDT experiments, and exact target journal are settled.
...
| What is the real novelty beyond P2, 960 input, and common attention modules? | TOFC evidence if complete; otherwise method selection protocol and ablation tables | If TOFC works, present its validated contribution; if not, frame as systematic high-resolution lightweight analysis. | Add/finish TOFC or another validated module; otherwise avoid strong novelty claims. |
...
| Does the method generalize beyond VisDrone? | UAVDT results only if complete | Claim generalization only after UAVDT is complete; otherwise state that cross-dataset validation is future/pending. | Complete UAVDT conversion and baseline/final-method runs. |
...
> We have limited the generalization claim to the datasets for which complete evidence is available. If UAVDT results are not complete, we present cross-dataset validation as future work rather than as a manuscript contribution.
...
This response plan is preparatory. It cannot replace missing evidence for TOFC, UAVDT, official AP-small, or multi-seed stability.
