...
CSGATE_RUNS = [
    ...
    ("CSGate experiment", "UAVDT CSGate complete result", "yolo11n_p2_csgate_960_uavdt"),
]
...
def render_report(checks: list[Check]) -> str:
    ...
    local_rest_status = (
        "OPEN_SECOND_CYCLE_EXPERIMENTS"
        if missing == 0 and non_result_status == "CLOSED_EXCEPT_RESULT_AND_MANUAL_GATES"
        else "OPEN_LOCAL_NON_RESULT_ITEMS"
    )
    submission_status = (
        "NOT_READY_WAITING_FOR_EXPERIMENT_AND_MANUAL_GATES"
        if pending or missing
        else "READY_FOR_FINAL_SUBMISSION_REVIEW"
    )
